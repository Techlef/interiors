...
alt.on('consoleCommand', (command, ...args) => {
	if(command == 'settimecycle' + string){
		game.setTimecycleModifier(string);
	}
});
...
alt.on('connectionComplete', (mapChanged) => {
  ...
  alt.on('consoleCommand', (command, ...args) => {
	if(command == 'createPed') {
	  alt.log('creating new ped......');
	  alt.loadModel(-1555576182)
	  game.createPed(4, -1555576182, 0, 0, 72, 0, false, true)
	  alt.log('ped is created!!!');
	}
  });

  game.setClockTime(12, 00, 00);
...
game.clearAmbientZoneState('AZ_COUNTRYSIDE_PRISON_01_ANNOUNCER_GENERAL', 1, 0); // Turn off prison sound
game.clearAmbientZoneState('AZ_COUNTRYSIDE_PRISON_01_ANNOUNCER_WARNING', 1, 0); // Turn off prison sound
game.clearAmbientZoneState('AZ_COUNTRYSIDE_PRISON_01_ANNOUNCER_ALARM', 1, 0); // Turn off prison sound
game.setAmbientZoneState(0, 0, 0); // Set ambiant sound to 0,0,0
game.clearAmbientZoneState('AZ_DISTANT_SASQUATCH', 0, 0);
